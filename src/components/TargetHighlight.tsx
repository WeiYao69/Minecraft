...
export function TargetHighlight() {
  ...
  }, [playerFeetPosition, playerHeight, hotbar, selectedHotbarSlot, removeInventory, addDroppedItem]);
  ...
  const attemptPlaceBlock = () => {
    ...
    const currentSlot = state.hotbar[state.selectedHotbarSlot];
    
    if (targetBlock && targetNormal && currentSlot && currentSlot.type !== null && currentSlot.count > 0) {
      ...
      if (!checkIntersection(playerAABB, blockAABB)) {
        addBlock(placeX, placeY, placeZ, currentSlot.type);
      ...
      if (mouseState.current.isLeftDown) {
        ...
        if (elapsed >= breakTime) {
          ...
          if (canHarvest && props.drops) {
            playerStateRef.current.addDroppedItem(props.drops, [targetBlock.x, targetBlock.y, targetBlock.z], props.dropCount);
          }
        }
      } else {
        if (isMiningRef.current) {
